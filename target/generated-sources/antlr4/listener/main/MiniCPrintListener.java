...
public class MiniCPrintListener extends MiniCBaseListener {
	...
	public void exitIf_stmt(MiniCParser.If_stmtContext ctx) {
		...
		if (ctx.getChildCount() > 5) {
			// 현재 if_stmt 가 ELSE stmt 이면
			if (ctx.stmt(1).getParent().getParent().getParent() instanceof MiniCParser.If_stmtContext) {
				s1 = " " + ctx.IF().getText();
			}

			if (ctx.stmt(1).getChild(0) instanceof MiniCParser.If_stmtContext) {
				// ELSE stmt 에서 stmt 가 if_stmt 이면
				s6 = " " + ctx.ELSE().getText() + newTexts.get(ctx.stmt(1));
